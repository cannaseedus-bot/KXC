...
int main(){
    ...
    double aEmbed=0,aRecord=0,aFlush=0,aRead=0; std::vector<double> tickMs;
    g_binds=0; g_syncs=0; g_upB=0; g_dnB=0;                 // isolate per-tick generation cost
    for(UINT t=0;t<Tn;t++){
        double p0=nowMs();
        ...
        double p4=nowMs();
        aEmbed+=p1-p0; aRecord+=p2-p1; aFlush+=p3-p2; aRead+=p4-p3; tickMs.push_back(p4-p0);
    ...
    long long gen_binds=g_binds, gen_syncs=g_syncs; double gen_up=g_upB, gen_dn=g_dnB;   // capture before end-KV readback
    ...
    // resident byte accounting (STATIC / GROWING / TRANSIENT)
    double staticB = (double)L*( (double)3*E*E + E*E + (double)E*H + (double)H*E + 8.0*E + 2.0*H )*4.0   // weights+biases/layer (approx)
                   + (double)2*E*4 + (double)E*V*4;                                                     // ln_f + lm_head
    double growB   = (double)L*2.0*Tn*Hn*Hd*4.0;                                                        // final KV, all layers
    double transB  = (double)(11*E + 2*H + V)*4.0;                                                      // scratch buffers
    double sumMs=0; for(double x:tickMs) sumMs+=x; double meanMs=sumMs/tickMs.size();
    printf("\n=== #004 BASELINE (measure before changing) ===\n");
    printf("[latency] compile(up-front)=%.1f ms | per-tick: embed=%.2f record/bind=%.2f exec+sync=%.2f readback+argmax=%.2f (mean/tick, ms)\n",
           tCompile, aEmbed/Tn, aRecord/Tn, aFlush/Tn, aRead/Tn);
    printf("[latency] first-token=%.1f ms  mean/token=%.1f ms  total(gen)=%.1f ms\n", tickMs[0], meanMs, sumMs);
    printf("[ops]     compiles=%d (up-front, 0/token in loop) | binds/token=%.0f | syncs/token=%.1f | up=%.1f KB/token dn=%.1f KB/token\n",
           nCompiles, (double)gen_binds/Tn, (double)gen_syncs/Tn, gen_up/Tn/1024.0, gen_dn/Tn/1024.0);
    printf("[resident] STATIC=%.0f MB (weights, immutable)  GROWING=%.3f MB (KV, append)  TRANSIENT=%.3f MB (scratch)\n",
           staticB/1e6, growB/1e6, transB/1e6);
    printf("[targets] the two repeated per-tick setup costs to amortize in #004: (A) MHA recompiled per cache length P; (B) ~%.0f binding-table creations/token\n", (double)gen_binds/Tn);
