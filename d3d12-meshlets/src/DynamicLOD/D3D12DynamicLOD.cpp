...
// ── D3D-1 Hive World Bridge ──────────────────────────────────────────────────
// Reads optional Assets/world.bin written by PYIDE-1/D3D-1 micronaut.
// Format: header(16 bytes) + N × WorldEntry(32 bytes)
// Falls back to grid layout if file absent.
#pragma pack(push, 1)
struct WorldHeader { char magic[4]; uint32_t version; uint32_t count; uint32_t flags; };
struct WorldEntry  { float px, py, pz, radius; float scale, rot_y, pad0, pad1; };
#pragma pack(pop)

static std::vector<WorldEntry> LoadWorldBin()
{
    std::ifstream f(L"..\\Assets\\world.bin", std::ios::binary);
    if (!f) return {};
    WorldHeader hdr{};
    f.read(reinterpret_cast<char*>(&hdr), sizeof(hdr));
    if (hdr.magic[0]!='D'||hdr.magic[1]!='3'||hdr.magic[2]!='D'||hdr.magic[3]!='W') return {};
    std::vector<WorldEntry> entries(hdr.count);
    f.read(reinterpret_cast<char*>(entries.data()), hdr.count * sizeof(WorldEntry));
    return entries;
}
// ────────────────────────────────────────────────────────────────────────────
...
void D3D12DynamicLOD::RegenerateInstances()
{
    ...
    // ── D3D-1 Hive World Bridge: prefer world.bin if present ─────────────────
    auto worldEntries = LoadWorldBin();
    if (!worldEntries.empty())
    {
        // Clamp to allocated instance count
        const uint32_t wCount = std::min(static_cast<uint32_t>(worldEntries.size()), m_instanceCount);
        m_instanceCount = wCount;
        for (uint32_t i = 0; i < wCount; ++i)
        {
            const auto& we = worldEntries[i];
            XMVECTOR location = XMVectorSet(we.px, we.py, we.pz, we.radius);
            XMMATRIX world = XMMatrixScaling(we.scale, we.scale, we.scale)
                           * XMMatrixRotationY(we.rot_y)
                           * XMMatrixTranslation(we.px, we.py, we.pz);
